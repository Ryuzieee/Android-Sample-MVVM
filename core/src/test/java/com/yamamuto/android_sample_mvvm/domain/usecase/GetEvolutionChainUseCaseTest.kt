...
class GetEvolutionChainUseCaseTest {
    ...
    private fun fakeSpecies(jaName: String, chainUrl: String = "chain-url") = PokemonSpeciesModel(
        japaneseName = jaName, flavorText = "", genus = "", eggGroups = emptyList(),
        genderRate = -1, captureRate = 45, habitat = null, generation = "generation-i",
        evolutionChainUrl = chainUrl,
    )

    @Test
    fun `returns evolution stages with Japanese names`() = runTest {
        coEvery { getSpeciesUseCase("bulbasaur") } returns Result.success(fakeSpecies("フシギダネ", "chain-url"))
        coEvery { repository.getEvolutionChainByUrl("chain-url") } returns Result.success(
            listOf(
    ...
    @Test
    fun `returns failure when species fetch fails`() = runTest {
    ...
    @Test
    fun `returns failure when evolution chain fetch fails`() = runTest {
    ...
    @Test
    fun `falls back to empty Japanese name when species lookup fails`() = runTest {
        coEvery { getSpeciesUseCase("bulbasaur") } returns Result.success(fakeSpecies("フシギダネ", "chain-url"))
        coEvery { repository.getEvolutionChainByUrl("chain-url") } returns Result.success(
