...
class GetAbilityJapaneseNameUseCaseTest {
    ...
    @Test
    fun `returns Japanese name when ja key exists`() = runTest {
        coEvery { repository.getAbilityLocalizedNames("overgrow") } returns Result.success(
    ...
    @Test
    fun `falls back to ja-hrkt when ja not available`() = runTest {
        coEvery { repository.getAbilityLocalizedNames("overgrow") } returns Result.success(
    ...
    @Test
    fun `falls back to ability name when no Japanese available`() = runTest {
        coEvery { repository.getAbilityLocalizedNames("overgrow") } returns Result.success(
    ...
    fun `returns failure when repository fails`() = runTest {
        coEvery { repository.getAbilityLocalizedNames(any()) } returns Result.failure(
